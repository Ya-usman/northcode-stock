...
function toast({ title, description, variant, action }: Omit<ToasterToast, 'id' | 'open'>) {
  const id = genId()
  const update = (props: Partial<ToasterToast>) => dispatch({ type: 'UPDATE_TOAST', toast: { ...props, id } })
  const dismiss = () => dispatch({ type: 'DISMISS_TOAST', toastId: id })
  dispatch({
    type: 'ADD_TOAST',
    toast: { title, description, variant, action, id, open: true, onOpenChange: open => { if (!open) dismiss() } },
  })
