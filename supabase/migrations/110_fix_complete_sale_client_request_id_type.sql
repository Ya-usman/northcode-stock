-- ============================================================
-- 110 — Corrige le type de complete_sale.p_client_request_id
-- ============================================================
-- La migration 109 déclarait p_client_request_id en uuid, en supposant
-- (comme la migration 073 le supposait avant sa correction en 099) que le
-- client génère toujours un vrai UUID. Mais sales.client_request_id est du
-- type text depuis la migration 099 — précisément pour accepter les
-- identifiants "local-<timestamp>-<random>" de la file d'attente hors-ligne
-- au moment de la synchro, qui ne sont pas des UUID valides. La comparaison
-- `client_request_id = p_client_request_id` échouait donc systématiquement
-- avec "operator does not exist: text = uuid", quelle que soit la qualité
-- de la connexion — d'où le repli automatique vers la sauvegarde hors-ligne
-- observé sur chaque vente depuis le déploiement de la migration 109.
--
-- CREATE OR REPLACE FUNCTION ne peut pas changer le type d'un paramètre
-- existant (Postgres traiterait ça comme une surcharge distincte plutôt que
-- comme un remplacement) — on supprime donc explicitement l'ancienne
-- signature avant de recréer la fonction avec le bon type.
