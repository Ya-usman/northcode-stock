-- ============================================================
-- 112 — Échéance de paiement pour les ventes à crédit
-- ============================================================
-- Une date par vente (pas par client — un client peut avoir plusieurs
-- ventes à crédit avec des échéances différentes). Retard calculé à la
-- volée côté client (comme purchase_orders.expected_delivery_date,
-- migration 101) — aucun trigger, aucun statut à maintenir.

ALTER TABLE sales ADD COLUMN IF NOT EXISTS due_date date;

-- Délai par défaut en jours, utilisé pour pré-remplir l'échéance au moment
-- de l'encaissement — modifiable dans Réglages, jamais obligatoire.
ALTER TABLE shops ADD COLUMN IF NOT EXISTS default_credit_term_days int NOT NULL DEFAULT 30;

-- complete_sale (109, corrigée 110) : ajoute p_due_date en fin de liste avec
-- DEFAULT NULL — CREATE OR REPLACE conserve le même OID (donc les GRANT déjà
-- en place) tant que le nouveau paramètre est en dernière position et a une
-- valeur par défaut ; pas de DROP FUNCTION nécessaire cette fois. Corps
-- inchangé par rapport à 110, sauf due_date ajoutée à l'INSERT de la vente.
CREATE OR REPLACE FUNCTION complete_sale(
  ...
  p_due_date           date DEFAULT NULL  -- échéance de paiement, pertinente seulement si la vente laisse un solde
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION complete_sale(
  ...
  text, text, text, text, jsonb, jsonb, date
) FROM PUBLIC;
