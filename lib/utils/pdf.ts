...
export async function generateReportPDF(params: {
  ...
  doc.setFontSize(13)
  doc.setFont('helvetica', 'bold')
  doc.text('StockShop', margin + 18, 12)
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('Generated by StockShop — northcode-stock.vercel.app', margin, 291)
...
/** Same as generateReportPDF but returns a Blob instead of auto-downloading */
export async function generateReportPDFBlob(params: {
  shopName: string
  dateRange: string
  sections: { title: string; headers: string[]; rows: (string | number)[][] }[]
}): Promise<{ blob: Blob; fileName: string }> {
  // Re-use buildReceiptDoc pattern — temporarily swap doc.save by capturing output
  const { jsPDF } = await import('jspdf')
  const autoTable = (await import('jspdf-autotable')).default
  ...
  return {
    ...
    fileName: `Report-${shopName}-${dateRange}.pdf`,
  }
}
