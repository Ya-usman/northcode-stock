...
export default function RegisterPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      // Wipe ALL caches before registering a new account
      await supabase.auth.signOut()
      localStorage.clear()
      // Clear service worker caches so the new account never sees stale data
      if ('caches' in window) {
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <button type="button" onClick={signInWithApple}
                    ...
                    {tAuth('continue_apple')}
                  </button>
                </div>

                <div className="flex items-center gap-3">
                  <div className="flex-1 h-px bg-border dark:bg-white/20" />
                  <span className="text-xs text-muted-foreground uppercase tracking-wider">{tAuth('or_separator')}</span>
                  <div className="flex-1 h-px bg-border dark:bg-white/20" />
                </div>
